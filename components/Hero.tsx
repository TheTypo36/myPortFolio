import React from "react";
import { Spotlight } from "./ui/Spotlight";
import { TextGenerateEffect } from "./ui/TextGenerateEffect";
import { MagicButton } from "./ui/MagicButton";
import { FaLocationArrow } from "react-icons/fa";
import { TypewriterEffect } from "./ui/TyperwriterEffect";

const Hero = () => {
  return (
    <div className="pb-20 pt-36">
      <Spotlight
        className="-top-40 -left-10 md:-left-32 md:-top-20 h-screen"
        fill="white"
      />
      <Spotlight className="-top-10 left-full h-[80vh] w-[50vw] fill=purple" />
      <Spotlight className="top-28 left-80  h-[80vh] w-[50vw]" fill="blue" />
      <div className="h-screen w-full dark:bg-black-100 bg-white  dark:bg-grid-white/[0.03] bg-grid-black/[0.2] absolute top-0 left-0 flex items-center justify-center">
        {/* Radial gradient for the container to give a faded look */}
        <div className="absolute pointer-events-none inset-0 flex items-center justify-center dark:bg-black-100 bg-white [mask-image:radial-gradient(ellipse_at_center,transparent_20%,black)]" />
      </div>
      <div className="flex justify-center relative 2xl:left-80 xl:left-20 lg:-left-20 md:-left-30 my-20 z-10">
        <div className="max-w-[89vw] md:mx-w-2xl lg:max-w-[60vw] flex flex-col items-center justify-center">
          <h2 className="uppercase tracking-widest text-xs text-center text-blue-100 max-w-80">
            Dynamic Web Magic with Nextjs
          </h2>
          <TextGenerateEffect
            className="text-center text-{40px} md:text-5xl lg:text-6xl"
            words="Transforming Concepts into Seamless User Experiences"
          />
          <TypewriterEffect
            words={[
              { text: "Hi," },
              { text: "I'm " },
              {
                text: "Ashish,",
                className: "text-blue-500 dark:text-blue-500",
              },
              { text: "a" },
              { text: "next" },
              { text: "js" },
              { text: "Developer" },
              { text: "based" },
              { text: "in" },
              { text: "Delhi," },
              { text: "India.", className: "text-blue-500 dark:text-blue-500" },
            ]}
          ></TypewriterEffect>
          <a href="#about" className="mt-5 sm:mt-3">
            <MagicButton
              title="show my work"
              icon={<FaLocationArrow />}
              position="right"
            />
          </a>
        </div>
      </div>
    </div>
  );
};

export default Hero;
